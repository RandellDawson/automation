...
const getPrComments = async number => {
  let data = await paginate(
    octokit.issues.listComments,
    { number, ...methodProps }
  );

  const issueComments = data.map(({
    user: { login: commenter }, body, updated_at }
  ) => ({ commenter , updated_at}));

  data = await paginate(
    octokit.pullRequests.listComments,
    { number, ...methodProps }
  );
  const prComments = data.map(({
    user: { login: commenter }, body, updated_at }
  ) => ({ commenter, updated_at }));

  return { issueComments, prComments };
};

const getMembers = async () => {
  const data = await paginate(
    octokit.orgs.listMembers,
    { org: 'freeCodeCamp', page: 1, per_page: 100 }
  );
  return data.reduce((members, { login }) => {
    members[login] = true;
    return members;
  }, {});
};
