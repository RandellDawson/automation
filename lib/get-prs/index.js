...
const { getLastPrProcessed } = require('../utils');
...
const getUserInput = async(rangeType = '') => {
  let data, firstPR, lastPR, lastPrevPR;
  if (rangeType === 'all') {
    ...
    firstPR = data[0];
    lastPR = data[1];
    if (type !== 'new'  && type !== 'all' && type !== 'range') {
      throw 'Please specify either new, all or range for 1st arg.';
    }
    if (type === 'range') {
      ...
      lastPrevPR = getLastPrProcessed();
      if (lastPrevPR < lastPR) {
        firstPR = +lastPrevPR + 1;
      }
    }
  }
